...
if exist "build-package-simple.ps1" (
    echo ✅ 找到简化PowerShell脚本
    powershell -ExecutionPolicy Bypass -File "build-package-simple.ps1" -Version %VERSION% -OutputDir %OUTPUT_DIR%
) else if exist "scripts\build-package.ps1" (
    echo ✅ 找到PowerShell脚本
    powershell -ExecutionPolicy Bypass -File "scripts\build-package.ps1" -Version %VERSION% -OutputDir %OUTPUT_DIR%
) else (
    echo ❌ PowerShell脚本不存在
    echo 请确保以下文件之一存在:
    echo - build-package-simple.ps1
    echo - scripts\build-package.ps1
    pause
    exit /b 1
)
